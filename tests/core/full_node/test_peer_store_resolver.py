...
@pytest.mark.parametrize("with_selected_network", [True, False])
def test_resolve_unmodified_legacy_peer_db_path(with_selected_network: bool, tmp_path: Path):
    """
    ...
    resolver: PeerStoreResolver = PeerStoreResolver(
        ...
        default_peers_file_path=("db/peers_CHALLENGE.dat" if with_selected_network else "db/peers.dat"),
    )
    # Expect: peers.dat path has the same directory as the legacy db
    assert resolver.peers_file_path == root_path / Path(
        "db/peers_mainnet.dat" if with_selected_network else "db/peers.dat"
    )
    # Expect: the config is updated with the new value
    assert config["peers_file_path"] == os.fspath(
        Path("db/peers_CHALLENGE.dat" if with_selected_network else "db/peers.dat")
    )
...
# use tmp_path pytest fixture to create a temporary directory
@pytest.mark.parametrize("with_selected_network", [True, False])
def test_resolve_modified_legacy_peer_db_path(with_selected_network: bool, tmp_path: Path):
    """
    ...
    # Expect: peers.dat path has the same directory as the legacy db
    assert resolver.peers_file_path == root_path / Path(
        "some/modified/db/path/peers_mainnet.dat" if with_selected_network else "some/modified/db/path/peers.dat"
    )
    # Expect: the config is updated with the new value
    assert config["peers_file_path"] == os.fspath(
        Path(
            "some/modified/db/path/peers_CHALLENGE.dat" if with_selected_network else "some/modified/db/path/peers.dat"
        )
    )
    # Expect: the config retains the legacy peer_db_path value
...
# use tmp_path pytest fixture to create a temporary directory
@pytest.mark.parametrize("with_selected_network", [True, False])
def test_resolve_default_peers_file_path(with_selected_network: bool, tmp_path: Path):
    """
    ...
    root_path: Path = tmp_path
    config: Dict[str, str] = {"peers_file_path": "db/peers_CHALLENGE.dat" if with_selected_network else "db/peers.dat"}
    resolver: PeerStoreResolver = PeerStoreResolver(
        root_path,
        config,
        selected_network="mainnet",
    ...
    assert resolver.peers_file_path == root_path / Path("db/peers_mainnet.dat")
    # Expect: the config is updated with the new value
    assert config["peers_file_path"] == "db/peers_CHALLENGE.dat" if with_selected_network else "db/peers.dat"
...
# use tmp_path pytest fixture to create a temporary directory
@pytest.mark.parametrize("with_selected_network", [True, False])
def test_resolve_modified_peers_file_path(with_selected_network: bool, tmp_path: Path):
    """
    ...
    root_path: Path = tmp_path
    config: Dict[str, str] = {
        "peers_file_path": "some/modified/db/path/peers_CHALLENGE.dat"
        if with_selected_network
        else "some/modified/db/path/peers.dat"
    }
    resolver: PeerStoreResolver = PeerStoreResolver(
        root_path,
        config,
    ...
    # Expect: peers.dat path is the same as the location specified in the config
    assert resolver.peers_file_path == root_path / Path("some/modified/db/path/peers_mainnet.dat")
    # Expect: the config is updated with the new value
    assert (
        config["peers_file_path"] == "some/modified/db/path/peers_CHALLENGE.dat"
        if with_selected_network
        else "some/modified/db/path/peers.dat"
    )
    # Expect: the config doesn't add a legacy peer_db_path value
...
@pytest.mark.parametrize("with_selected_network", [True, False])
def test_resolve_both_peers_file_path_and_legacy_peer_db_path_exist(with_selected_network: bool, tmp_path: Path):
    """
    ...
    config: Dict[str, str] = {
        ...
        "peers_file_path": "db/peers_CHALLENGE.dat" if with_selected_network else "db/peers.dat",
    }
    resolver: PeerStoreResolver = PeerStoreResolver(
        root_path,
        config,
...
@pytest.mark.parametrize("with_selected_network", [True, False])
def test_resolve_modified_both_peers_file_path_and_legacy_peer_db_path_exist(
    with_selected_network: bool, tmp_path: Path
):
    """
...
# use tmp_path pytest fixture to create a temporary directory
@pytest.mark.parametrize("with_selected_network", [True, False])
def test_resolve_missing_keys(with_selected_network: bool, tmp_path: Path):
    """
...
# use tmp_path pytest fixture to create a temporary directory
@pytest.mark.parametrize("with_selected_network", [True, False])
def test_resolve_with_testnet(with_selected_network: bool, tmp_path: Path):
    """
    ...
    # Expect: the config is updated with the new value
    assert config["peers_file_path"] == os.fspath(
        Path("db/peers_CHALLENGE.dat" if with_selected_network else "db/peers_testnet123.dat")
    )
    # Expect: the config doesn't add a legacy peer_db_path value
    assert config.get("peer_db_path") is None
...
# use tmp_path pytest fixture to create a temporary directory
@pytest.mark.parametrize("with_selected_network", [True, False])
def test_resolve_default_legacy_db_path(with_selected_network: bool, tmp_path: Path):
    """
